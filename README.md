...
!!! Решил использовать для графиков апи `CryptoCompare`, так как апи позволяет просматриравть с точностью до дня, часа, минуты.
Логику получения данных из предосталенных апи `GetChartPoint` в `CoinCap`.
В CoinGesko OHLC возвращяеться в виде массива масивов `/coins/{id}/ohcl`.
