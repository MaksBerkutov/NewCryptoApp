...
Было решенно использовать для графиков API `CryptoCompare`. Т
ак как он позволяет просматриравть пары  дня, часа, минуты.

Логику получения данных из предосталенных API я сделал в `CoinCap` функция  `GetChartPoint` .
Не делал дла CoinGesko так как OHLC возвращяеться в виде массива масивов `/coins/{id}/ohcl`, как по мне неудобно разбирать.

## Реализованный функционал

Полный список функционлал можно просмотреть в `TODO.md`

- Отображение топ N валют (количество регулируеться).
- Логика многостраничного приложения.
- Навигация
- MVVM 
- Поиск (Монеты, Обмены, Нфт)
- Подробная информация (Монетах, Обменниках, Нфт)
- Работа с 3 API
    - CoinCap
    - CoinGesko
    - CryptoCompare
- Графики
  - Несколько вариантов отобрежения графиков
     ...
     - Scatter
     - Signal
  - Отображение объема валюты на отдельном графике
  - Выбор получения пар (Минута, День, Час)
...
|   |   |---BaseAPI.cs                  // Базовый класс который представляет возврощяемый объект
|   |---UI/                             // Кастомные UI
|   |   |---Template/                   // Шаблоны вывода для DTO
|   |---MVVM/                           // Файлы MVVM Паттерна
|   |   |---View/                       // Представления MVVM Паттерна
|   |   |---ViewModel/                  // Отображения MVVM Паттерна
|   |---Lang/                           // Файлы для смены языка (Контроллер, и словари к нему)
|   |---Theme/                          // Файлы для смены темы (Контроллер, и словари к нему)
...
-  [x] Дефолтное отображение ошибок юзеру
...
```
Так же если ваше Api возврощяет данные обвёрнутые в `Data` или вам нужно только определённое поле, то можно использовать конвертор.
...
public static async Task<ModelDTO> GetValute()
{
    ...
    {
        // template это динамический объект результата с API 
        return template.data.ToString(); //Достаём только поле data
    });
}
```
...
Back - механизм позволяющий упростить перход на преведущии страници.

Он не хранит состоянии страниц их нужно хранить отдельно если нужно, например в `Store`.
Back введёт запись переходов пользователя, и позволяет упростить механизм перехода назад.
Для его работы нужно объязательно указать хотя бы одну статическую страницу (как минимум главная страница).
При переходе на любую из главных страниц история переходов обнуляеться и заново начинает записываться уже текущие перходы пользователя.
Вам не нужно отслеживать откудого может прийти пользователь и куда ему нужно вернуться, данный механизм сделаёт все сам.

Как использовать?
-
Всё просто просто укажите статичиские страници.
```cs
Navigate.RegisterStaticPage<MainPageView>();
```
Добавьте новые страници в проект, и реализуйте переходы
...
Теперь в любой из страниц добавьте механизм который будет перенаправлять назад (например кнопка).
...
И пользователя гарантированно перекинет туда от куда он пришёл.



### Store

Локальное  временное хранилище для пердачи данных между приложением

Для того что бы положить данные нужно зарегистрировать их
```cs
Store.Register(Data);
```
Тип данных поддтянеться автоматически.

Для получения данных есть несколько варинтов. 
Первый функция `GetOrNull<T>` которая вернёт в случае не нахождения ваших данных null.
Вторая же `Get<T>()` вызовет исключение.
```cs
var dataOrNull = Store.GetOrNull<DataType>();
var datal = Store.Get<DataType>();
```

Так же можно полностью очистить хранилище для этого есть функция `ClearAll`
```cs
Store.ClearAll();
```
